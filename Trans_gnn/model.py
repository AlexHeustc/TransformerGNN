...
# from torch_geometric.graphgym import GCNConv,GATConv
...
class GATGNN_R(torch.nn.Module):
    ...
    def forward(self,x, edge_index, edge_attr,batch, global_feat,cluster):
        ...
        y         = gdp(x,batch).unsqueeze(1).squeeze()
...
class TransformerGNN(torch.nn.Module):
    def __init__(self,heads,neurons=64,nl=3,xtra_layers=True
        ...
        # Extracting values from the new format
        # n_h = neurons
        self.n_heads = heads
        self.n_layers = nl
        self.concat_comp = concat_comp
        ...
        for i in range(self.n_layers):
            self.conv_layers.append(TransformerConv(in_channels=n_h,
                                     out_channels=n_h,
                                     heads=heads,
                                     dropout=0.3,
                                     edge_dim=n_h,
        ...
        if self.additional:
            self.linear1    = nn.Linear(reg_h,reg_h)
            self.linear2    = nn.Linear(reg_h,reg_h)
        # All other parameters remain the same as in original TransformerGNN


        # Linear layers
        self.linear1 = Linear(n_h, n_h)
        self.linear2 = Linear(n_h, n_h)
        self.linear3 = Linear(n_h * 3,n_h)
    ...
    def forward(self,x, edge_index, edge_attr,batch, global_feat,cluster):
        ...
        for i in range(self.n_layers):
            ...
            x = torch.relu(self.lin_layers[i](x))
